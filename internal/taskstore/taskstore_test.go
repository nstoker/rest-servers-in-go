...
func TestCreateAndGet(t *testing.T) {
	ts := New()
	id := ts.CreateTask("Bonjour", nil, time.Now())
	...
	if task.Text != "Bonjour" {
		t.Errorf("get Text '%v', want '%v'", task.Text, "Bonjour")
	}
...
func TestDelete(t *testing.T) {
	...
	allTasks := ts.GetAllTasks()
	if len(allTasks) != 0 {
		t.Fatalf("deleted all tasks individually, len(allTasks)==%d, want 0", len(allTasks))
	}
}
...
func TestDeleteAll(t *testing.T) {
	...
	if len(tasks) != 0 {
		t.Fatalf("all tasks deleted, but have %d tasks", len(tasks))
	}
}
...
func TestGetTasksByTag(t *testing.T) {
	...
	ts.CreateTask("YWZ", []string{"Bills"}, time.Now())
	ts.CreateTask("WZT", []string{"Movies"}, time.Now())
	...
	for _, tt := range tests {
		t.Run(tt.date, func(t *testing.T) {
			...
			if numByDate != tt.wantNum {
				t.Errorf("for %v: got %v, want %v", tt.date, numByDate, tt.wantNum)
			}
		})
	}
}
